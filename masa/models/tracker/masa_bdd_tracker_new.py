...
class MasaBDDTracker(BaseTracker):
    ...
    Args:
        ...
        memo_momentum_fast (float): Fast prototype momentum. Defaults to 0.7.
        memo_momentum_slow (float): Slow prototype momentum. Defaults to 0.02.
        logit_scale (float): Logit scale for matching. Defaults to 10.0.
    """
    ...
    def __init__(
        ...
        distractor_score_thr: float = 0.5,  # Distractor score threshold
        distractor_nms_thr: float = 0.3,  # Distractor NMS threshold
        ...
        for id, bbox, embed, label, score in zip(
            ...
            if id in self.tracks.keys():
                ...
                self.tracks[id] = dict(
                    ...
                    embed_fast=z0.clone(),
                    embed_slow=z0.clone(),
                    embed=z0.clone(),  # Keep for compatibility
                    label=label,
                    score=score,
                    last_frame=frame_id,
                    velocity=torch.zeros_like(bbox),
                    acc_frame=0,
                    video_id=int(video_id),  # EMD: 保存 video_id
                    first_frame=int(frame_id),  # EMD: 记录首帧
                    mem_bank=[{  # EMD: 初始化 memory bank
                        "feat": z0.detach().cpu(),
                        "score": float(score.detach().cpu()),
                        "frame": int(frame_id),
    ...
    def memo(self) -> Tuple[Tensor, ...]:
        ...
        for backdrop in self.backdrops:
            ...
            # For backdrops, use the same embedding for both fast and slow
            memo_embeds_fast.append(backdrop["embeds"])
            memo_embeds_slow.append(backdrop["embeds"])
    ...
    def track(
        ...
        **kwargs
        ...
        if bboxes.size(0) > 0 and not self.empty:
            (memo_bboxes, memo_labels, memo_embeds_fast, memo_embeds_slow, memo_ids, memo_vs) = self.memo
            ...
            scale = getattr(self, "logit_scale", 10.0)
        ...
        # 获取 video_id
        video_id = metainfo.get("video_id", metainfo.get("vid_id", -1))
