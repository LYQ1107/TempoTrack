...
EVENT_CACHE="${EVENT_CACHE:-/data2/usr_for_deadline/tempotrack_v9_relocated_20260910/masa_r50_vov_aligned/test/event_cache}"
VAL_RESULT="${VAL_RESULT:-$BASE/aligned_val_frozen_trained.json}"
CHECKPOINT="${CHECKPOINT:-/data2/usr_for_deadline/tempotrack_v9_relocated_20260910/masa_r50/training/training/seed0}"
MERGED="$BASE/aligned_test_base_adapted_trained.json"
...
while [ ! -s "$EVENT_CACHE/metadata.json" ] || [ ! -s "$VAL_RESULT" ]; do
  sleep 60
done

for index in 0 1 2 3; do
  output="$BASE/aligned_test_base_adapted_trained_retry_s${index}.json"
  [ -s "$output" ] && continue
  wait_mem
  ...
  "$PY" -m tempotrack_research.cli psmr-v9 sweep-psmr \
    ...
    --output "$output" \
    >"$BASE/aligned_test_base_adapted_trained_retry_s${index}.log" 2>&1
done

LD_PRELOAD="${LD_PRELOAD:-/home/lwr/anaconda3/envs/masaenv/lib/libsqlite3.so.3.52.0}" \
"$PY" -m tempotrack_research.cli psmr-v9 merge-sweep \
  --repo "$REPO" \
  --part "$BASE/aligned_test_base_adapted_trained_retry_s0.json" \
  --part "$BASE/aligned_test_base_adapted_trained_retry_s1.json" \
  --part "$BASE/aligned_test_base_adapted_trained_retry_s2.json" \
  --part "$BASE/aligned_test_base_adapted_trained_retry_s3.json" \
  --output "$MERGED"
