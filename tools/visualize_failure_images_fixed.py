#!/usr/bin/env python
"""Generate image visualizations for tracking failure cases with proper scaling."""
import argparse
import json
import os
...
def parse_args():
    ...
    parser.add_argument('--output-dir', default='results/rebuttal_figures/failure_cases_fixed')
    parser.add_argument('--iou-threshold', type=float, default=0.5)
    parser.add_argument('--num-cases', type=int, default=10)
    parser.add_argument('--frames-per-case', type=int, default=6)
...
def draw_boxes_on_frame(img, anns, scale=1.0):
    for ann in anns:
        ...
        cv2.rectangle(img, (x, label_y - text_h - 3), (x + text_w + 4, label_y + 3), color, -1)
...
def create_comparison_grid(frames, gt_tracks, pred_tracks, gt_img_map, img_root, output_path, title):
    ...
    scale = 480 / orig_w if orig_w > 480 else 1.0
    new_w, new_h = int(orig_w * scale), int(orig_h * scale)
    margin, label_h, title_h = 10, 40, 60
    canvas_w = 2 * new_w + 3 * margin
    canvas_h = title_h + len(frames) * (new_h + label_h + margin) + margin
    canvas = np.ones((canvas_h, canvas_w, 3), dtype=np.uint8) * 255
    cv2.putText(canvas, title, (margin, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
    cv2.putText(canvas, "Ground Truth", (margin, title_h + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 150, 0), 2)
    cv2.putText(canvas, "Prediction", (new_w + 2 * margin, title_h + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 200), 2)
    for idx, img_id in enumerate(frames):
        ...
        gt_frame = draw_boxes_on_frame(img.copy(), [a for t in gt_tracks.values() for a in t if a['image_id'] == img_id], scale)
        pred_frame = draw_boxes_on_frame(img.copy(), [a for t in pred_tracks.values() for a in t if a['image_id'] == img_id], scale)
        ...
        cv2.putText(canvas, f"Frame {img_info['frame_index']}", (margin, y_offset - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (100, 100, 100), 1)
...
def main():
    ...
    for video_id in tqdm(list(gt_tracks_by_video.keys())):
        ...
        for case in find_id_switches(pred_tracks, gt_tracks, gt_img_map, args.iou_threshold, args.max_objects):
            case['video_id'] = video_id
            all_cases['id_switches'].append(case)
        for case in find_fragmentations(pred_tracks, gt_tracks, gt_img_map, args.iou_threshold, args.max_objects):
            case['video_id'] = video_id
    ...
    for case_type, cases in all_cases.items():
        ...
        for case in cases:
            ...
            title = f"ID Switch: GT#{case['gt_id']}" if case_type == 'id_switches' else f"Fragmentation: GT#{case['gt_id']}"
            if create_comparison_grid(frames, gt_tracks_by_video[video_id], pred_tracks_by_video.get(video_id, {}), gt_img_map, args.img_root, type_dir / f"case_{created + 1}.jpg", title):
                created += 1
