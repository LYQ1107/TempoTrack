#!/usr/bin/env python
"""
Generate video visualizations for tracking failure cases.
...
def parse_args():
    ...
    parser.add_argument('--output-dir', default='results/rebuttal_figures/failure_videos')
    parser.add_argument('--iou-threshold', type=float, default=0.5)
    parser.add_argument('--num-cases', type=int, default=10)
    parser.add_argument('--video-length', type=int, default=30, help='Number of frames per video')
        ...
        if len(unique) > 1:
            for i in range(1, len(pred_ids)):
                if pred_ids[i][1] != pred_ids[i-1][1]:
                    switch_frame = pred_ids[i][2]
                    start_frame = max(0, i - 15)
                    end_frame = min(len(pred_ids), i + 15)

                    switches.append({
                        'gt_id': gt_id,
                        'switch_frame': switch_frame,
                        'frames': [pred_ids[j][0] for j in range(start_frame, end_frame)],
...
def find_fragmentations(pred_tracks, gt_tracks, img_map, iou_thresh, max_objects):
    ...
    for gt_id, gt_anns in gt_tracks.items():
        ...
        if len(matched) > 1:
            ...
            mid = len(gt_sorted) // 2
            start = max(0, mid - 15)
            end = min(len(gt_sorted), mid + 15)

            frags.append({
                'gt_id': gt_id,
                'frames': [gt_sorted[i]['image_id'] for i in range(start, end)],
...
def draw_boxes_on_frame(img, anns, is_gt=False):
...
def create_comparison_video(frames, gt_tracks, pred_tracks, gt_img_map, img_root, output_path, fps, title):
    """Create side-by-side GT vs Prediction video."""
    if not frames:
        return False
    ...
    for img_id in frames:
        ...
        gt_frame = draw_boxes_on_frame(gt_frame, gt_anns, is_gt=True)
        pred_frame = draw_boxes_on_frame(pred_frame, pred_anns, is_gt=False)

        canvas = np.ones((out_h, out_w, 3), dtype=np.uint8) * 255

        cv2.putText(canvas, title, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 0), 2)

        cv2.putText(canvas, "Ground Truth", (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 150, 0), 2)
        cv2.putText(canvas, "Prediction", (new_w + 20, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 200), 2)

        canvas[80:80+new_h, 0:new_w] = gt_frame
        canvas[80:80+new_h, new_w+20:new_w+20+new_w] = pred_frame
...
def main():
    ...
    for case_type, cases in all_cases.items():
        ...
        for i, case in enumerate(cases):
            ...
            frames = case['frames'][:args.video_length]
            if len(frames) < 10:
                continue
            ...
            output_path = type_dir / f"case_{created + 1}.mp4"

            success = create_comparison_video(
                frames, gt_tracks, pred_tracks, gt_img_map,
                args.img_root, output_path, args.fps, title
            )

            if success:
                created += 1
                print(f"  Created {case_type} video {created}/{args.num_cases}")

    summary = output_dir / "failure_summary.txt"
    with open(summary, 'w') as f:
        f.write("Failure Case Video Summary\n" + "="*80 + "\n\n")
        f.write(f"Generated {args.num_cases} videos per failure type\n")
        f.write(f"Video length: {args.video_length} frames at {args.fps} FPS\n")
        f.write(f"Max objects per frame: {args.max_objects} (to avoid clutter)\n\n")
        f.write("Each video shows:\n")
        f.write("  - Left: Ground Truth with color-coded IDs\n")
        f.write("  - Right: Prediction with color-coded IDs\n")
